...
import (
	...
	"gitlab.com/stevealexrs/celo-client-lite-go/contractkit/client"
...
// New creates a new contracts Registry with ethclient
func New(eth *ethclient.Client) (Registry, error) {
	registry, err := contracts.NewRegistry(RegistryAddress, eth)
...
func (r *registryImpl) tryParseProxyLog(log types.Log) (eventName string, event interface{}, err error) {
	proxy, err := contracts.NewProxyFilterer(log.Address, r.Eth)
	if err != nil {
		return "", nil, err
	}
